    ...
    def _run_single(
        ...
        *,
        ...
        while True:
            ...
            state = self.client.status().get(session_id, {}).get("type")
            if state in (None, "idle"):
                ...
                if len(messages) > baseline:
                    ...
                    log.info(
                        "session %s completed in %.1fs (%d chars)", session_id, elapsed, len(text)
                    )
                    return RunResult(text=text, session_id=session_id)
            if time.monotonic() >= deadline:
                self.abort_session(handle)
                raise TimeoutError(f"session {session_id} exceeded {timeout}s (state={state})")

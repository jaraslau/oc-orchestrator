...
def gh_ok(args: list[str]) -> tuple[int, str, str]:
    return 0, "", ""


PR_JSON_ONE: str = json.dumps(
    [
        {
            "number": 7,
...
def reviewing_task(repo: Path) -> dict[str, Any]:
    task: dict[str, Any] = service.create_task(repo, title="Thing")
...
class TestGhClient:
    def test_pr_number_from_url(self) -> None:
    ...
    def test_check_failure_raises(self) -> None:
        def handler(a: list[str]) -> tuple[int, str, str]:
    ...
    def test_create_parses_url(self) -> None:
        def handler(args: list[str]) -> tuple[int, str, str]:
        ...
        flat: list[str] = [a for call in calls for a in call]
        assert "--delete-branch" not in flat

    def test_merge_uses_method_flag(self) -> None:
        seen: list[list[str]] = []
    ...
    def test_comment_passes_body(self) -> None:
        ...
        flat: list[str] = [a for call in seen for a in call]
        assert "fix it" in flat and "3" in flat


class TestPrLifecycle:
    def test_open_creates_and_records(self, repo: Path) -> None:
        ...
        runner, calls = make_runner(handler)
        result: dict[str, Any] = service.open_pr(repo, task["id"], gh_runner=runner)
        assert result["pr"]["number"] == 11
        assert result["task"]["status"] == "PR_OPEN"
        flat: list[str] = [a for c in calls for a in c]
        assert "--base" in flat and "main" in flat

    def test_open_reuses_existing_pr(self, repo: Path) -> None:
        task: dict[str, Any] = reviewing_task(repo)
    ...
    def test_request_changes_posts_and_marks(self, repo: Path) -> None:
        task: dict[str, Any] = reviewing_task(repo)
        ledger_path: Path = repo / ".orchestrator" / "ledger.json"
        ...
        runner, _ = make_runner(handler)
        result: dict[str, Any] = service.request_changes(
            repo, task["id"], "rotate tokens please", gh_runner=runner
        )
    ...
    def test_request_changes_without_pr_still_marks(self, repo: Path) -> None:
        task: dict[str, Any] = reviewing_task(repo)
        runner, calls = make_runner(gh_ok)
        result: dict[str, Any] = service.request_changes(
            repo, task["id"], "redo it", gh_runner=runner
        )
        assert result["posted_to_pr"] is False
        assert result["task"]["status"] == "CHANGES_REQUESTED"
        assert calls == []

    def test_merge_happy_path_cleans_worktree(self, repo: Path, tmp_path: Path) -> None:
        task: dict[str, Any] = reviewing_task(repo)
        ledger_path: Path = repo / ".orchestrator" / "ledger.json"
        ledger = Ledger.load(ledger_path)
        t = ledger.get(task["id"])
        ...
        ledger.save()
        wt: Path = repo / ".orchestrator" / "worktrees" / t.branch
    ...
    def test_merge_blocked_by_unmet_dependency(self, repo: Path) -> None:
        base: dict[str, Any] = service.create_task(repo, title="Base")
        child: dict[str, Any] = service.create_task(repo, title="Child", dependencies=[base["id"]])
        ledger = Ledger.load(repo / ".orchestrator" / "ledger.json")
    ...
    def test_pr_diff(self, repo: Path) -> None:
        task: dict[str, Any] = reviewing_task(repo)
        ledger = Ledger.load(repo / ".orchestrator" / "ledger.json")
        t = ledger.get(task["id"])
        t.pr = "https://github.com/acme/repo/pull/7"
        ...
        runner, _ = make_runner(handler)
        out: str = service.pr_diff(repo, task["id"], gh_runner=runner)
        assert out.startswith("diff --git")

    def test_list_open_prs(self, repo: Path) -> None:
        def handler(args: list[str]) -> tuple[int, str, str]:
            return 0, PR_JSON_ONE, ""

        runner, _ = make_runner(handler)
        rows: list[dict[str, Any]] = service.list_open_prs(repo, gh_runner=runner)
        assert rows[0]["number"] == 7 and rows[0]["head"] == "agent/task-001-thing"


class TestReport:
    def test_report_sections(self, repo: Path) -> None:
        a: dict[str, Any] = service.create_task(repo, title="Done thing")
        b: dict[str, Any] = service.create_task(repo, title="Failed thing")
        ledger = Ledger.load(repo / ".orchestrator" / "ledger.json")
        ...
        text: str = service.generate_report(repo)
    ...
    def test_empty_ledger_report(self, repo: Path) -> None:
...
class TestDispatchInstructions:
    ...
    def test_dispatch_passes_instructions_to_worker_prompt(self, repo: Path) -> None:
        ...
        log: Path = repo / ".orchestrator" / "logs" / f"{task['id'].lower()}.log"
